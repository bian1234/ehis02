 ...
 */
package com.thinkgem.jeesite.cfq.common.service;

import com.google.common.collect.Lists;
import com.thinkgem.jeesite.cfq.common.persistence.BaseEntity;
...
public abstract class BaseService {
     ...
     * @return 标准连接条件对象
     */
    public static String dataScopeFilter1(PrescriptionUser prescriptionUser, String officeAlias, String userAlias) {
        ...
        if (!prescriptionUser.isAdmin()) {
            ...
            for (PrescriptionRole r : prescriptionUser.getRoleList()) {
                for (String oa : StringUtils.split(officeAlias, ",")) {
                    if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)) {
                        if (PrescriptionRole.DATA_SCOPE_ALL.equals(r.getDataScope())) {
                            isDataScopeAll = true;
                        } else if (PrescriptionRole.DATA_SCOPE_COMPANY_AND_CHILD.equals(r.getDataScope())) {
                            sqlString.append(" OR " + oa + ".id = '" + prescriptionUser.getCompany().getId() + "'");
                            sqlString.append(" OR " + oa + ".parent_ids LIKE '" + prescriptionUser.getCompany().getParentIds() + prescriptionUser.getCompany().getId() + ",%'");
                        } else if (PrescriptionRole.DATA_SCOPE_COMPANY.equals(r.getDataScope())) {
                            sqlString.append(" OR " + oa + ".id = '" + prescriptionUser.getCompany().getId() + "'");
                            // 包括本公司下的部门 （type=1:公司；type=2：部门）
                            sqlString.append(" OR (" + oa + ".parent_id = '" + prescriptionUser.getCompany().getId() + "' AND " + oa + ".type = '2')");
                        } else if (PrescriptionRole.DATA_SCOPE_OFFICE_AND_CHILD.equals(r.getDataScope())) {
                            sqlString.append(" OR " + oa + ".id = '" + prescriptionUser.getOffice().getId() + "'");
                            sqlString.append(" OR " + oa + ".parent_ids LIKE '" + prescriptionUser.getOffice().getParentIds() + prescriptionUser.getOffice().getId() + ",%'");
                        } else if (PrescriptionRole.DATA_SCOPE_OFFICE.equals(r.getDataScope())) {
                            sqlString.append(" OR " + oa + ".id = '" + prescriptionUser.getOffice().getId() + "'");
                        } else if (PrescriptionRole.DATA_SCOPE_CUSTOM.equals(r.getDataScope())) {
...
//							}
            ...
            if (!isDataScopeAll) {
                if (StringUtils.isNotBlank(userAlias)) {
                    for (String ua : StringUtils.split(userAlias, ",")) {
                        sqlString.append(" OR " + ua + ".id = '" + prescriptionUser.getId() + "'");
                    }
                } else {
                    for (String oa : StringUtils.split(officeAlias, ",")) {
     ...
     * @return 标准连接条件对象
     */
    public static String dataScopeFilter(PrescriptionUser user, String officeAlias, String userAlias) {
        ...
        if (!user.isAdmin()) {
            boolean isDataScopeAll = false;
            for (PrescriptionRole r : user.getRoleList()) {
                for (String oa : StringUtils.split(officeAlias, ",")) {
                    if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)) {
                        if (PrescriptionRole.DATA_SCOPE_ALL.equals(r.getDataScope())) {
                            isDataScopeAll = true;
...
//							}
    ...
    public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
        ...
        PrescriptionUser user = entity.getCurrentUser();
        ...
        for (String where : StringUtils.split(officeWheres, ",")) {
            if (PrescriptionRole.DATA_SCOPE_COMPANY_AND_CHILD.equals(dataScopeString)) {
                // 包括本公司下的部门 （type=1:公司；type=2：部门）
            ...
            } else if (PrescriptionRole.DATA_SCOPE_COMPANY.equals(dataScopeString)) {
                sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
                sqlString.append(" WHERE type='2'");
                sqlString.append(" AND id = '" + user.getCompany().getId() + "'");
                sqlString.append(" AND " + where + ")");
            } else if (PrescriptionRole.DATA_SCOPE_OFFICE_AND_CHILD.equals(dataScopeString)) {
                sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
                ...
                sqlString.append(" AND " + where + ")");
            } else if (PrescriptionRole.DATA_SCOPE_OFFICE.equals(dataScopeString)) {
                sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
                sqlString.append(" WHERE id = '" + user.getOffice().getId() + "'");
                sqlString.append(" AND " + where + ")");
            } else if (PrescriptionRole.DATA_SCOPE_CUSTOM.equals(dataScopeString)) {
        ...
        for (String where : StringUtils.split(userWheres, ",")) {
            if (PrescriptionRole.DATA_SCOPE_SELF.equals(dataScopeString)) {
