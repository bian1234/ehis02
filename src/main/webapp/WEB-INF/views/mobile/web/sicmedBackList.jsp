				...
				<div class="container-fluid">
					<div class="navbar-header">
						<a class="navbar-brand"
							href="${ctx}/webweb/sicmedRegistered/backList">退号列表</a> 
					</div>
				</div>
			</nav>
			<!--</div>-->
		...
		<%-- <form:form id="searchForm" modelAttribute="sicmedRegistered"
			...
			<ul class="ul-form">
				<li><label>患者名称：</label> <form:input
						path="sicmedPatient.patientName" htmlEscape="false" maxlength="6" style="height:34px;"
						class="span3"/></li>
				<li><label>患者编号：</label> <form:input
						path="sicmedPatient.patientNumber" htmlEscape="false" style="height:34px;"
						maxlength="32" class="span3" /></li>
				<li class="btns"><input id="btnSubmit" class="btn btn-primary" 
					type="submit" value="查询" /></li>
				<li class="clearfix"></li>
			</ul>
		</form:form> --%>
		<form id="searchForm" action="${ctx}/webweb/sicmedRegistered/backList"
			method="post" class="breadcrumb form-search">
			<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
			<input id="pageSize" name="pageSize" type="hidden"
				value="${page.pageSize}" /> <i class="icon-search">&nbsp;&nbsp;</i>
			<input id="String" class="selectpicker" name="regist" maxlength="32"
				style="height: 34px;" placeholder="请输入患者编号或名字"> <input
				id="searchForm" class="btn btn-primary" type="submit" value="查询" />
		</form>
			...
			<tbody>
				<c:forEach items="${page.list}" var="sicmedRegistered">
					<tr>
						...
						<td><a
							href="${ctx}/webweb/sicmedRegistered/back?id=${sicmedRegistered.id}&doctor=${doctor}"><input
								class="btn btn-default" type="button" value="退号" /></a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
