...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/web/sicmedRegistered/">保存挂号信息成功列表</a></li>
		<li class="active"><a href="${ctx}/web/sicmedRegistered/form?id=${sicmedRegistered.id}">保存挂号信息成功<shiro:hasPermission name="rest:sicmedRegistered:edit">${not empty sicmedRegistered.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="rest:sicmedRegistered:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="sicmedRegistered" action="${ctx}/web/sicmedRegistered/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<%-- <div class="control-group">
			<label class="control-label">医生(对应医生表)：</label>
			<div class="controls">
				<form:input path="sicmedDoctor.id" htmlEscape="false" maxlength="32" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div> --%>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="rest:sicmedRegistered:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
